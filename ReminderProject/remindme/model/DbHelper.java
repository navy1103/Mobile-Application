...
//	private static final String TAG = "DbHelper";
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		if (oldVersion < 2) {
			db.execSQL("ALTER TABLE " + Alarm.TABLE_NAME + " ADD COLUMN " + Alarm.COL_INSISTENT + " INTEGER DEFAULT 0;");
		}
		if (oldVersion < 3) {
			db.execSQL("ALTER TABLE " + Alarm.TABLE_NAME + " ADD COLUMN " + Alarm.COL_TAG + " TEXT;");
			db.execSQL("ALTER TABLE " + Alarm.TABLE_NAME + " ADD COLUMN " + Alarm.COL_RINGTONE + " TEXT;");
			db.execSQL("ALTER TABLE " + Alarm.TABLE_NAME + " ADD COLUMN " + Alarm.COL_VIBRATE + " INTEGER DEFAULT 1;");
		}		
	}
...
//		    	Log.e(TAG, e.getMessage(), e);
	...
	/**
	 * @param db
	 * @param args {startTime, endTime, tag}
	 * @return cursor
	 */	
	public Cursor listNotifications(SQLiteDatabase db, String... args) {
		...
		selection += (args!=null && args.length>2 && args[2]!=null) ? " AND a."+Alarm.COL_TAG+" = '"+args[2]+"'" : "";
	...
	public int cancelNotification(SQLiteDatabase db, long alarmId, String startTime) {
		ContentValues cv = new ContentValues(1);
		cv.put(AlarmMsg.COL_STATUS, AlarmMsg.CANCELLED);
		return db.update(AlarmMsg.TABLE_NAME, 
							cv, 
							AlarmMsg.COL_ALARMID+" = ? AND "+AlarmMsg.COL_DATETIME+" >= ?", 
							new String[]{String.valueOf(alarmId), startTime});
	}	
	
	private final String listAlarmsSQL = Util.concat("SELECT count(*) count, ",
															"a."+Alarm.COL_NAME+" "+Alarm.COL_NAME+", ",
															"a."+Alarm.COL_ID+" "+Alarm.COL_ID+", ",
															"a."+Alarm.COL_FROMDATE+" "+Alarm.COL_FROMDATE+", ",
															"a."+Alarm.COL_TODATE+" "+Alarm.COL_TODATE,
													" FROM "+Alarm.TABLE_NAME+" AS a",
													" JOIN "+AlarmMsg.TABLE_NAME+" AS am",
													" ON a."+Alarm.COL_ID+" = am."+AlarmMsg.COL_ALARMID);

	public Cursor listAlarms(SQLiteDatabase db, Boolean isActive) {
		String selection = "1=1";
		String[] selectionArgs = null;
		
		if (isActive != null) {
			selection += " AND am."+AlarmMsg.COL_STATUS+" = '" + (isActive ? AlarmMsg.ACTIVE : AlarmMsg.EXPIRED) + "'";
			selection += " AND am."+AlarmMsg.COL_DATETIME + (isActive ? " > " : " < ") +"?";
			
			selectionArgs = new String[]{String.valueOf(System.currentTimeMillis())};
		}
		
		String sql = Util.concat(listAlarmsSQL,
								" WHERE "+selection,
								" GROUP BY a."+Alarm.COL_ID,
								" ORDER BY a."+Alarm.COL_ID+" DESC");
	 ...
	 public static final String[] getDateTokens(String dateStr) {
	 ...
	 public static final String[] getTimeTokens(String timeStr) {
