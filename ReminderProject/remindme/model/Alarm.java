...
public class Alarm extends AbstractModel {
	...
	public static enum TAG {ALARMS, APPOINTMENTS, BILLS, BIRTHDAYS, CALLS, EVENTS, WORKOUTS, MEETINGS, PILLS};//GROCERIES, TASKS
	...
	long save(SQLiteDatabase db) {
		...
		cv.put(COL_INSISTENT, insistent ? 1 : 0);
		cv.put(COL_TAG, tag);
		cv.put(COL_RINGTONE, ringtone);
		cv.put(COL_VIBRATE, vibrate ? 1 : 0);
	...
	public boolean load(SQLiteDatabase db) {
		...
		try {
			if (cursor.moveToFirst()) {
				...
				insistent = cursor.getInt(cursor.getColumnIndex(COL_INSISTENT)) == 1 ? true : false;
				tag = cursor.getString(cursor.getColumnIndex(COL_TAG));
				ringtone = cursor.getString(cursor.getColumnIndex(COL_RINGTONE));
				vibrate = cursor.getInt(cursor.getColumnIndex(COL_VIBRATE)) == 1 ? true : false;
				return true;
			}
			return false;
		} finally {
	...
	private Boolean vibrate = Boolean.FALSE;
